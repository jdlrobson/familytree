...
function listRoots(trees) {
  const lis = trees.map((tree) => {
    const root = tree.root;
    return `<li><a href="${tree.root.id}.html">${root.id}</a>
      (born ${date(root.data.dob)}, died ${date(root.data.dod)})</li>`;
  }).join('\n');
  return `<h1>Family trees</h1><ul>${lis}</ul>`
}

function saveTreeToHTML(filename, trees, rootsOnly) {
  const content = rootsOnly ? listRoots(trees) :
